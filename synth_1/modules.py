...
def AttentionWidth (rate):
    print rate[:, : 1, :].shape, rate[:, : -1, :].shape
    left = nd.concat (rate[:, : 1, :], rate[:, : -1, :], dim = 1)
    right = nd.concat (rate[:, 1 :, :], rate[:, -1 :, :], dim = 1)
    v=0.5 * rate + 0.25 * (left + right)
    v=v[0].asnumpy()
    loc_time=0
    for i,value in enumerate(v):
        loc_time+=(int(value[0]+0.5)*0.05)
        print loc_time
...
class End2End_1 (gluon.nn.HybridBlock):
    ...
    def hybrid_forward (self, F, p,  seg,  T, dy_dec, values_L, mask_enc, ratio):      
        ...
        #x_embed = F.concat (p_embed, t_embed, o_embed, dim = 2)  
        #x_embed_rate = F.concat (p_embed_rate, t_embed_rate, o_embed_rate, dim = 2)
        ...
        #y = self.dec_conv_3 (y) 
        y = self.dense_dec_1 (attention * F.expand_dims (mask_dec, axis = 2))

        y = self.dense_dec_2 (y)

        y = self.dense_dec_3 (y) 
