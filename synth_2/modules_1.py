...
class Attention (gluon.HybridBlock):
	...
	#print index
	#print [mx.nd.sum(index[:,:,i]).asnumpy()[0] for i in range(index.shape[-1])]
...
class Relative (gluon.HybridBlock):
	...
	'''
	plt.plot(relative_posenc_1[0,:500,0].asnumpy())
	plt.plot(relative_posenc_2[0,:500,0].asnumpy())
	plt.plot(relative_posenc_3[0,:500,0].asnumpy())
	plt.show()	
        '''
...
class End2End_1 (gluon.nn.HybridBlock):
    ...
    def __init__ (self, num_phoneme, num_down_enc, num_down_dec,dim_embed_seg,dim_embed_phoneme, size_enc, size_dec, size_output, dp_dec, prefix):
	    ...
	    self.emd_seg_rate = gluon.nn.Dense (self.dim_embed_seg, flatten = False, prefix = 'dense_seg_rate')
	...
	s_embed_rate = self.emd_seg_rate(seg)
        ...
        sum_out = rate_x.reshape(-1, 1)* mask_enc.reshape(-1, 1)
        	
	return attention, F.sum (sum_out), atten_index, rate_x
        #rel_info = self.rel (atten_index, mask_enc, mask_dec, T, y_index)

        #mask_dec = F.expand_dims (mask_dec, axis = 2)

        #return self.U_Net_dec (self.dense_dec (F.concat (attention, rel_info, dim = 2)) * mask_dec), rate_x 
