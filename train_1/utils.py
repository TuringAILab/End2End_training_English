...
class data_iterator (object):
    ...
    def __init__ (self,json_file, aco_path, batch_size, enc_num_down, file_rate_info, num_phonemes, aco_shape, if_sort = True, sort_rule = 'input'):
    ...
    def load_one_batch (self):
        ...
        #print self.data[self.keys[i]]['token_phoneme']#,input_len_needed,'input_len_needed'
        encoderLenNeed = np.power (2, self.enc_num_down) # Since the input length is restricted to be smaller than this value     
	input_len_needed = ((max_len_input // encoderLenNeed) + 1) * encoderLenNeed
        ...
        for i in range (self.global_index, self.global_index + self.batch_size):
            ...
            #print aco.shape, self.aco_shape
            dec_mask = np.ones (shape = aco.shape[0]) 

            output_lengths.append (aco.shape[0])
            #print aco.shape , self.data[self.keys[i]]['length_output'],self.keys[i],'aco.shape'
        ...
        return np.array (token_phonemes),  np.array (segs), np.array (enc_masks), np.array (acos), np.array (dec_masks), np.array (output_lengths), np.array (ratios)
